...
def get_model(model_name, total_num_vals, L, var_info, D, M, natural, scale, device):
    if model_name == 'VAE':
        return VAE(total_num_vals=total_num_vals, L=L, var_info=var_info, D=D, M=M, natural=natural, scale=scale, device=device)
