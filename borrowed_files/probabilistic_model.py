...
def _get_distributions(dists_names) -> List[Base]:
    ...
    for i, name in enumerate(dists_names):
        # only 'categorical' (implemented) and 'ordinal' (not implemented) has parameter 'size' = num_probs.
            # this is retrieved through string
        ...
        # get_distribution_by retrieves the class, and initializes it based on num_probs (if categorical / ordinal). 
        if num_probs == 1:
...
class ProbabilisticModel(object):
    def __init__(self, dists_names):
        self.dists = _get_distributions(dists_names) # initializing distribution classes based on list of dists. 
